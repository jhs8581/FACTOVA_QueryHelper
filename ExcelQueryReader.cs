...
namespace FACTOVA_Palletizing_Analysis
{
    public class QueryItem
    {
        ...
        // 새로운 옵션 필드 속성들
        public string EnabledFlag { get; set; } = string.Empty; // G열: 'Y'이면 실행 가능
        public string NotifyFlag { get; set; } = string.Empty; // H열: 'Y'이면 알림 표시
        public string CountGreaterThan { get; set; } = string.Empty; // I열: 건 수 이상일 때 알림
        public string CountEquals { get; set; } = string.Empty; // J열: 건 수와 같을 때 알림
        public string CountLessThan { get; set; } = string.Empty; // K열: 건 수 이하일 때 알림
        public string ColumnNames { get; set; } = string.Empty; // L열: 체크할 컬럼명 (A,B,C 형식)
        public string ColumnValues { get; set; } = string.Empty; // M열: 체크할 값들 (1,2,3 형식)
    ...
    public class ExcelQueryReader
    {
            ...
            string excludeColumn = "N")
        {
            ...
            try
            {
                ...
                if (queryColIndex == 0)
                {
                    throw new ArgumentException("쿼리 열이 지정되지 않았습니다.");
                }
